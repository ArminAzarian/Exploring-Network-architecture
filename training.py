...
from model_definitions import ConstantParamTanh, VAE, Discriminator, combined_loss, create_synthetic_video
...
# --- Training Loop ---
def train_constant_tanh(alpha_mode, device="cpu"):

    # --- Model Initialization ---
    tanh_layer = ConstantParamTanh(VIDEO_HEIGHT, VIDEO_WIDTH, mode = alpha_mode) #Single, Matrix or TimeFunction
    vae = VAE().to(device)
    ...
    print(f"Training complete for ConstantParamTanh model with {alpha_mode} mode")

# --- Main execution ---
if __name__ == "__main__":
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # Run with different alpha modes
    train_constant_tanh("Single", device)
    train_constant_tanh("Matrix", device)
    #For the Time function variant it needs the t variable.
